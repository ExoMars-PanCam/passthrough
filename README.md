...
`fetch` is in effect.
##### When fetching
In the context of a `fetch`, `required` is used to determine whether the failure to find a corresponding source element
should be treated as an error condition (True), or if the element (subtree) should instead be omitted from the output 
product (False). A common usage example of this feature can be seen in the **Overview** section above.
##### When not fetching
In the non-`fetch`ing context, `required` signals whether an element left empty by the client (and/or any `fill` 
evaluations; see below) should be treated as an error condition at the time of output label export (True), or if the 
element (subtree) can be pruned from the output product (False).

> Elements that are "nilled out", i.e. that contain the attribute `xsi:nil="true"`, are not considered empty by PT.

In the case where the `required` property of a class is evaluated to be false, the class will be removed from the
output label if any of its descendant attributes are found to be empty. However, this will result in a warning if
any of its descendants are populated (currently regardless of whether these were prepopulated with static values in
the template). The goal of issuing a warning is to minimise confusion around "mysteriously missing" classes when PT
is used with a human-in-the-loop (interactive) processor.
...
element is required to be present in the output label.

For example,
```xml
<emrsp_rm_pan:ISEM_Footprint pt:fetch="false()" pt:required="//ex:Some_Indicative/ex:attribute == 'ISEM'" pt:sources="template">
    <emrsp_rm_pan:x/>
    <emrsp_rm_pan:y/>
    <emrsp_rm_pan:radius/>
...
```
can be read as "the `emrsp_rm_pan:ISEM_Footprint` class will be pruned by the post-processor if any of `x`, `y` or 
`radius` are empty and `ex:Some_Indicative/ex:attribute` in the output label (conventionally set as the active source 
via the `template` source map key) is not equal to "ISEM".
